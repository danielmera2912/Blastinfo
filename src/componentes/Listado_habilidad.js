...
let hayContenido;
  ...
  <main className="cuerpo cuerpo--listado">
    ...
    {
    buscar == null && identidad=='habilidad' && filtro=="id_descendente" && pokemons != null && pokemons.results.length > 0 && [...pokemons.results].splice((paginaActual-1)*8,8).map(item => { 
      return( 
    ...
    {
    buscar == null && identidad=='habilidad' && filtro=="id_ascendente" && pokemons != null && pokemons.results.length > 0 && [...pokemons.results].reverse().splice((paginaActual-1)*8,8).map(item => { 
      return( 
        <Caja_habilidad key={item.name} name={item.name}/>
    )}) 
    }
    ...
    buscar == null &&  identidad=='habilidad' && recuerdaFiltro==0 && filtro=="nombre_descendente" && pokemons != null && pokemons.results.length > 0 && [...pokemons.results].sort((a, b) => {
    ...
    {
    buscar == null &&  identidad=='habilidad' && recuerdaFiltro==0 && filtro=="nombre_ascendente" && pokemons != null && pokemons.results.length > 0 && [...pokemons.results].sort((a, b) => {
      const nameA = a.name.toUpperCase();
      const nameB = b.name.toUpperCase(); 
      recuerdaFiltro++;
      if (nameA > nameB) {
